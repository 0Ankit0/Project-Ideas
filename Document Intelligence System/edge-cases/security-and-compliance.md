...
- Add model-risk controls: approval workflows, dataset access reviews, prompt/model change tickets, and retention boundaries for reviewer-visible sensitive data.
---
...
### Model Risk & Compliance Controls
- Maintain data processing inventory by document type and legal basis with region-aware residency controls.
- Require red-team prompts/tests for prompt-injection and data exfiltration pathways in document text.
- Enforce reversible tokenization/redaction pipeline before reviewer exposure for sensitive attributes.
- Keep signed governance records for every production model promotion and rollback decision.
