...
### BPMN Extensions
- Model human task queues, escalation timers, and policy decision gateways so operational ownership between AI services and reviewers is explicit.
---
...
### Mermaid: Swimlane (Ops + Human Review)
```mermaid
flowchart LR
    subgraph System
      A[Process Document] --> B[Evaluate Threshold]
      B --> C[Create Review Task]
    end
    subgraph Reviewer
      C --> D[Claim Task]
      D --> E[Correct Fields]
