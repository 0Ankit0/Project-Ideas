...
### System Sequence Extensions
- Include reviewer task generation and closure callbacks to ensure orchestration remains consistent between synchronous APIs and asynchronous worker pipelines.
---
...
sequenceDiagram
    ...
    participant M as ML Stack
    participant R as Review
    U->>G: Upload + Process
    G->>O: Start workflow
    O->>M: OCR/Classify/Extract
    M-->>O: predictions + confidence
    O->>O: apply policy
    alt Needs review
      O->>R: create task
      R-->>O: approved corrections
    end
