...
## Process Narrative (Operational failure playbook)
1. **Initiate**: Sre Lead captures the primary change request for **Operations** and links it to business objectives, impacted modules, and target release windows.
2. **Design/Refine**: The team elaborates flows, assumptions, acceptance criteria, and exception paths specific to operational failure playbook.
3. **Authorize**: Approval checks confirm that changes satisfy policy, architecture, and compliance constraints before promotion.
4. **Execute**: Ops Control Plane executes the approved path and enforces runbook guard checks at run-time or publication-time.
...
6. **Verify & Close**: Stakeholders reconcile expected outcomes against actual telemetry to confirm service continuity.

## Role/Permission Matrix (Operations)
| Capability | Employee | Manager | HR/People Ops | Engineering/IT | Compliance/Audit |
|---|---|---|---|---|---|
| View operations artifacts | Scoped self | Team scoped | Full | Full | Read-only full |
...
## State Model (Operational failure playbook)
```mermaid
stateDiagram-v2
    [*] --> Draft
...
## Integration Behavior (Operations)
