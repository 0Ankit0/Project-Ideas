...
## Process Narrative (Payroll/benefit exception handling)
1. **Initiate**: Payroll Lead captures the primary change request for **Payroll And Benefits** and links it to business objectives, impacted modules, and target release windows.
2. **Design/Refine**: The team elaborates flows, assumptions, acceptance criteria, and exception paths specific to payroll/benefit exception handling.
3. **Authorize**: Approval checks confirm that changes satisfy policy, architecture, and compliance constraints before promotion.
4. **Execute**: Payroll Core executes the approved path and enforces financial consistency checks at run-time or publication-time.
...
6. **Verify & Close**: Stakeholders reconcile expected outcomes against actual telemetry to confirm payout correctness.

## Role/Permission Matrix (Payroll And Benefits)
| Capability | Employee | Manager | HR/People Ops | Engineering/IT | Compliance/Audit |
|---|---|---|---|---|---|
| View payroll and benefits artifacts | Scoped self | Team scoped | Full | Full | Read-only full |
...
## State Model (Payroll/benefit exception handling)
```mermaid
stateDiagram-v2
    [*] --> Draft
...
## Integration Behavior (Payroll And Benefits)
