...
## Purpose
Define the cloud architecture artifacts for the **Payment Orchestration and Wallet Platform** with implementation-ready detail.
...
This document now defines **cloud-runtime** behavior and explicitly maps architecture intent to API contracts, diagrammed flows, and day-2 operations owned by **Platform SRE, Payments Eng**.
...
- `POST /payments`: documented here with required request ids, idempotency keys, and failure reason codes for **cloud-runtime**.
- `POST /ledger/journals`: documented here with required request ids, idempotency keys, and failure reason codes for **cloud-runtime**.
- `GET /payments/{id}`: documented here with required request ids, idempotency keys, and failure reason codes for **cloud-runtime**.
- `GET /reconciliation/discrepancies`: documented here with required request ids, idempotency keys, and failure reason codes for **cloud-runtime**.
- All mutating calls MUST return `correlation_id`, `idempotency_key`, `previous_state`, `new_state`, and `transition_reason`.

### In-depth flow diagram for cloud-architecture
```mermaid
sequenceDiagram
    autonumber
    participant Client
    ...
    Client->>API: create/capture request (cloud-runtime)
