...
This document now defines **delivery-tracking** behavior and explicitly maps architecture intent to API contracts, diagrammed flows, and day-2 operations owned by **Engineering Managers, TPM**.
...
- `POST /payments`: documented here with required request ids, idempotency keys, and failure reason codes for **delivery-tracking**.
- `POST /payments/{id}/capture`: documented here with required request ids, idempotency keys, and failure reason codes for **delivery-tracking**.
- `POST /reconciliation/runs`: documented here with required request ids, idempotency keys, and failure reason codes for **delivery-tracking**.
- `POST /ledger/journals`: documented here with required request ids, idempotency keys, and failure reason codes for **delivery-tracking**.
- All mutating calls MUST return `correlation_id`, `idempotency_key`, `previous_state`, `new_state`, and `transition_reason`.

### In-depth flow diagram for backend-status-matrix
```mermaid
sequenceDiagram
    autonumber
    participant Client
    ...
    Client->>API: create/capture request (delivery-tracking)
