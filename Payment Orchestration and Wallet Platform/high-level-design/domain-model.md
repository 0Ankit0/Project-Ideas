...
## Purpose
Define the domain model artifacts for the **Payment Orchestration and Wallet Platform** with implementation-ready detail.
...
This document now defines **domain-boundaries** behavior and explicitly maps architecture intent to API contracts, diagrammed flows, and day-2 operations owned by **Architecture, Domain Owners**.
...
- `POST /payments`: documented here with required request ids, idempotency keys, and failure reason codes for **domain-boundaries**.
- `POST /payouts`: documented here with required request ids, idempotency keys, and failure reason codes for **domain-boundaries**.
- `POST /payments/{id}/refunds`: documented here with required request ids, idempotency keys, and failure reason codes for **domain-boundaries**.
- `POST /ledger/journals`: documented here with required request ids, idempotency keys, and failure reason codes for **domain-boundaries**.
- All mutating calls MUST return `correlation_id`, `idempotency_key`, `previous_state`, `new_state`, and `transition_reason`.

### In-depth flow diagram for domain-model
```mermaid
sequenceDiagram
    autonumber
    participant Client
    ...
    Client->>API: create/capture request (domain-boundaries)
