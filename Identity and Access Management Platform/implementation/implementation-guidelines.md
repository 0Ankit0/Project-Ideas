...
- Use explicit domain modules: auth, session, token, policy, lifecycle, federation, audit.
- Enforce idempotency keys on all mutating external APIs.
- All external calls must define timeout, retry policy, and circuit breaker.

## Test Strategy
- Unit tests for state transitions and policy precedence semantics.
- Integration tests for federation handshake, SCIM drift handling, and token reuse detection.
- Contract tests for event schemas and decision API payloads.
- Chaos tests for cache outages and queue lag behavior.

## Secure Delivery Controls
- Required checks: SAST, dependency scan, IaC scan, secret scan.
- Feature flags for risky rollout with kill-switch behavior.
- Production change requires runbook link and rollback plan.
...
### Engineering Workflow Controls
- PR templates require threat-impact and audit-impact statements.
- Release trains include migration rehearsals in staging with production-like datasets.
- Incident learnings feed into coding standards and test regressions.
