...
```mermaid
flowchart LR
    API[Gateway + PEP] --> AUTH[Auth Component]
    API --> PDP[Policy Component]
    API --> LIFE[Lifecycle Component]

    AUTH --> FED[Federation Component]
    LIFE --> PROV[Provisioning Worker Component]
    PDP --> CACHE[Policy Cache]

    AUTH --> AUDIT[Audit Component]
    PDP --> AUDIT
    PROV --> AUDIT
```

## Dependency Rules
- PEP never bypasses PDP for protected resources.
- Worker components are isolated behind queue contracts.
- Audit writes are non-blocking but loss-intolerant (retry + DLQ).
...
### Dependency Policy
- Upstream API components never call worker internals directly.
- Audit component is write-only from domain services; reads occur via reporting APIs.
- Cross-component synchronous calls are minimized to keep failure domains isolated.
