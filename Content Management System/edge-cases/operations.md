...
## Scope
Runbook-level responses for production incidents and degraded dependencies.

## Incident Playbooks
1. **Event bus lag spike:** scale consumer group, pause non-critical projections, monitor recovery ETA.
2. **Database failover:** route traffic to standby, disable heavy admin reports, validate replication consistency.
3. **Third-party moderation outage:** switch to fallback ruleset and queue comments for delayed moderation.
4. **Cache corruption:** trigger selective warmup by top routes before full cache repopulation.
