...
## Scope
Client/server contract drift and rendering behavior inconsistencies.

## Scenarios and Mitigations
1. **Client behind API version:** support additive fields and compatibility shim for one major release.
2. **Pagination race:** cursor pagination with stable sort key avoids duplicates/missing rows.
3. **Editor preview mismatch:** render through same publishing template service used in production.
4. **Idempotency key reuse:** return prior response with warning metadata.
