...
## Scope
Exceptional states for review assignment, decisioning, and escalation.

## Scenarios and Mitigations
1. **Reviewer unavailable:** auto-delegate using on-call rotation map.
2. **SLA timer service outage:** reconstruct timers from durable task store and emit backlog alert.
3. **Conflicting decisions:** first successful CAS write wins; subsequent attempts return conflict.
4. **Policy change mid-review:** decision service re-evaluates against latest policy version before final transition.
