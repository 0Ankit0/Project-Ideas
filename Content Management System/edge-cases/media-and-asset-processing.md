...
## Scope
Upload, transformation, virus scanning, and delivery failure modes.

## Scenarios and Mitigations
1. **Large asset timeout:** use resumable multipart upload and checkpoint state.
2. **Virus scan false positive:** quarantine asset and provide reviewer override workflow.
3. **Thumbnail generation failure:** mark derivative `FAILED`, keep original, retry async with exponential backoff.
4. **CDN propagation lag:** serve stale-if-error and background purge retries.
