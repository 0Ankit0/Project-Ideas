# Publishing and Rollbacks

## Scope
Failure handling for publish pipelines and compensating rollback actions.

## Scenarios and Mitigations
1. **Partial multi-channel publish:** mark channel-specific status and retry failed channels only.
2. **Rollback target missing assets:** block rollback and suggest nearest safe revision.
3. **Feed regeneration timeout:** serve last known good feed and queue high-priority regeneration.
4. **Repeated rollback flapping:** enforce cool-down and require operator justification.
