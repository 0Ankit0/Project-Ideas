...
## Scope
Threat, abuse, and regulatory exceptions with required controls.

## Scenarios and Mitigations
1. **Compromised staff token:** revoke session family, require step-up MFA, and notify SOC.
2. **Suspicious publish burst:** anomaly detection throttles publish rights and opens incident.
3. **GDPR erasure request vs legal hold:** legal hold takes precedence; partial redaction applied.
4. **Audit log tampering attempt:** append-only store with signed digest chain prevents mutation.
