# Content Ingestion and Versioning

## Scope
Conflict handling and version integrity guarantees during rapid edits/imports.

## Scenarios and Mitigations
1. **Concurrent editor updates:** enforce `If-Match` and return merge base payload.
2. **Bulk import duplicates:** deterministic dedupe key (`tenant + externalId + hash`).
3. **Out-of-order event replay:** projection workers enforce monotonic revision version.
4. **Revision bloat:** archival policy compacts non-significant autosaves.
