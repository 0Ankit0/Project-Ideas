...
## Scope
HTTP and async contract design, versioning rules, and error semantics for all core workflows.

## API Conventions
- Base path: `/v1`.
- Idempotency required for mutating publish/rollback operations via `Idempotency-Key`.
- Concurrency control via `If-Match` header with revision version.
- Error format uses stable machine code + correlation id.

## Content API
| Endpoint | Purpose | AuthZ |
|---|---|---|
| `POST /v1/content` | Create draft | `content:create` |
| `PATCH /v1/content/{id}` | Update draft revision | `content:edit` |
| `POST /v1/content/{id}/submit` | Submit for review | `content:submit` |
| `GET /v1/content/{id}/preview` | Render preview | `content:view` |

## Workflow API
| Endpoint | Purpose |
|---|---|
| `GET /v1/workflow/tasks` | List open tasks with filters |
| `POST /v1/workflow/tasks/{id}/decision` | Approve/reject/delegate |
| `POST /v1/workflow/tasks/{id}/comment` | Add review comment |

## Publishing API
| Endpoint | Purpose |
|---|---|
| `POST /v1/publications/schedule` | Schedule publication |
| `POST /v1/publications/{id}/publish-now` | Force immediate publish |
| `POST /v1/publications/{id}/rollback` | Roll back publication |
...
```json
{
  "error": {
    ...
    "correlationId": "req-5f4f2e9e",
    "retryable": false,
    "details": {"taskId": "...", "expectedState": "OPEN"}
  }
}
```

## Async Webhook Contract (publish.completed)
```json
{
  ...
  "contentId": "...",
  "revisionId": "...",
  "publishedAt": "2026-03-28T14:10:00Z"
}
```
