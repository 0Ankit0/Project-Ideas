...
## Scope
Logical/physical relational schema, keys, and indexing for CMS data plane.

## DDL Snippets
```sql
CREATE INDEX idx_content_item_tenant_status ON content_item (tenant_id, status);
CREATE INDEX idx_workflow_task_assignee_state_due ON workflow_task (assignee_id, state, due_at);
CREATE UNIQUE INDEX uq_content_revision_version ON content_revision (content_id, version);
```

## Migration Strategy
1. Additive schema changes first.
2. Dual-write/dual-read during transition window.
3. Backfill historical data using batched workers.
4. Remove deprecated columns after 2 release cycles.
...
erDiagram
    ...
    CONTENT_ITEM ||--o{ WORKFLOW_TASK : routes
    CONTENT_ITEM ||--o{ PUBLISH_JOB : publishes
```
