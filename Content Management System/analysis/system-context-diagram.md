...
## Scope
External actor and system boundaries around CMS platform.

## Mermaid Diagram
```mermaid
flowchart LR
    Author((Author)) --> CMS[CMS Platform]
    Editor((Editor)) --> CMS
    Reader((Reader)) --> CDN[CDN + Web Delivery]
    CMS --> CDN
    CMS --> IDP[Identity Provider]
    CMS --> MOD[Moderation/Spam Service]
    CMS --> SEARCH[Search Service]
    CMS --> ANALYTICS[Analytics Warehouse]
    CMS --> MAIL[Email/Notification Provider]
