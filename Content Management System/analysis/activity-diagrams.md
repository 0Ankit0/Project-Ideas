...
## Scope
Control-flow diagrams for authoring, review, publish, and rollback.

## Mermaid Diagram
```mermaid
flowchart TD
    Start([Start]) --> Draft[Create Draft]
    Draft --> Validate{Metadata valid?}
    Validate -->|No| Fix[Show validation issues]
    Fix --> Draft
    Validate -->|Yes| Submit[Submit for review]
    Submit --> Decision{Approved?}
    Decision -->|No| Rework[Return with comments]
    Rework --> Draft
    Decision -->|Yes| Schedule{Schedule publish?}
    Schedule -->|Yes| Queue[Queue publication job]
    Schedule -->|No| Publish[Publish now]
    Queue --> Publish
    Publish --> End([End])
```
