...
## Scope
Field-level semantics, constraints, ownership, and retention definitions.

## Core Entities and Fields
| Entity.Field | Type | Constraints | Owner | Retention |
|---|---|---|---|---|
| `content_item.slug` | varchar(180) | unique by `tenant_id` + `locale`; regex `^[a-z0-9-]+$` | Content Team | life of content |
| `content_revision.body_json` | jsonb | max 2 MB; schema validated | Editor Platform | 18 months |
| `workflow_task.state` | enum | `OPEN|CLAIMED|RESOLVED|EXPIRED` | Workflow Team | 400 days |
| `media_asset.scan_status` | enum | `PENDING|CLEAN|BLOCKED` | Security Platform | 400 days |
| `audit_log.evidence_hash` | char(64) | SHA-256 immutable | Compliance Team | 7 years |

## Validation Contract Example
```json
{
  "field": "slug",
  "rule": "unique_per_tenant_locale",
  "severity": "error",
