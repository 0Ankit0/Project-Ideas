...
## Scope
Data transformations from ingestion through publication and analytics projections.

## Mermaid Diagram
```mermaid
flowchart TD
    In[Author Input] --> V[Validation + Normalization]
    V --> S[(Primary Store)]
    S --> O[Outbox]
    O --> B[(Event Bus)]
    B --> P[Publishing Projection]
    B --> X[Search Projection]
    B --> A[Analytics Projection]
    P --> C[CDN]
```
