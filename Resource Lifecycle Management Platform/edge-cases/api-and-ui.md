...
- **Provisioning in API and UI Edge Cases**: Define preconditions, policy gate, and emitted evidence artifact.
- **Allocation in API and UI Edge Cases**: Define contention handling, SLA timers, and rollback behavior.
- **Decommissioning in API and UI Edge Cases**: Define terminal checks, retention obligations, and approval authority.
- **Exception workflow in API and UI Edge Cases**: Detect → classify → contain → resolve → recover → postmortem with owner + SLA.
