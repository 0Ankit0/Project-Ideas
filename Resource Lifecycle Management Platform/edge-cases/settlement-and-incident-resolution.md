...
- **Provisioning in Settlement and Incident Resolution**: Define preconditions, policy gate, and emitted evidence artifact.
- **Allocation in Settlement and Incident Resolution**: Define contention handling, SLA timers, and rollback behavior.
- **Decommissioning in Settlement and Incident Resolution**: Define terminal checks, retention obligations, and approval authority.
- **Exception workflow in Settlement and Incident Resolution**: Detect → classify → contain → resolve → recover → postmortem with owner + SLA.
