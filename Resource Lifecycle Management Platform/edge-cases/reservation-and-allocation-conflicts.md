...
- **Provisioning in Reservation and Allocation Conflicts**: Define preconditions, policy gate, and emitted evidence artifact.
- **Allocation in Reservation and Allocation Conflicts**: Define contention handling, SLA timers, and rollback behavior.
- **Decommissioning in Reservation and Allocation Conflicts**: Define terminal checks, retention obligations, and approval authority.
- **Exception workflow in Reservation and Allocation Conflicts**: Detect → classify → contain → resolve → recover → postmortem with owner + SLA.
