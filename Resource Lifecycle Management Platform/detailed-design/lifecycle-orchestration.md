...
- **Provisioning in Lifecycle Orchestration**: Define preconditions, policy gate, and emitted evidence artifact.
- **Allocation in Lifecycle Orchestration**: Define contention handling, SLA timers, and rollback behavior.
- **Decommissioning in Lifecycle Orchestration**: Define terminal checks, retention obligations, and approval authority.
- **Exception workflow in Lifecycle Orchestration**: Detect → classify → contain → resolve → recover → postmortem with owner + SLA.
