...
- **Provisioning in Sequence Diagrams**: Define preconditions, policy gate, and emitted evidence artifact.
- **Allocation in Sequence Diagrams**: Define contention handling, SLA timers, and rollback behavior.
- **Decommissioning in Sequence Diagrams**: Define terminal checks, retention obligations, and approval authority.
- **Exception workflow in Sequence Diagrams**: Detect → classify → contain → resolve → recover → postmortem with owner + SLA.
...
sequenceDiagram
  participant Client
  participant AllocSvc
  participant LockSvc
  participant PolicySvc
  Client->>AllocSvc: allocate()
  AllocSvc->>LockSvc: lock(resource)
  AllocSvc->>PolicySvc: authorize()
  AllocSvc-->>Client: granted/denied
```
