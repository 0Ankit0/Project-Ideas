...
- **Provisioning in Network Infrastructure**: Define preconditions, policy gate, and emitted evidence artifact.
- **Allocation in Network Infrastructure**: Define contention handling, SLA timers, and rollback behavior.
- **Decommissioning in Network Infrastructure**: Define terminal checks, retention obligations, and approval authority.
- **Exception workflow in Network Infrastructure**: Detect → classify → contain → resolve → recover → postmortem with owner + SLA.
...
```mermaid
flowchart TB
  Internet --> DMZ --> AppSubnet --> DataSubnet
  AppSubnet --> SIEM
  DataSubnet --> Backup
```
