...
## Objective
Provide implementation-ready guidance for **Network Infrastructure** in the Messaging and Notification Platform.
...
flowchart LR
  ...
  CDN --> ALB[Public Load Balancer]
  ALB --> PrivateAPI[Private API Subnet]
  PrivateAPI --> App[Orchestration Services]
  App --> Queue[Private Queue Endpoint]
  App --> DB[Private DB Subnet]
  App --> NAT[NAT + Egress Firewall]
  NAT --> Providers[Provider Endpoints allow-list]
```
