...
## Objective
Provide implementation-ready guidance for **Activity Diagrams** in the Messaging and Notification Platform.
...
```mermaid
flowchart TD
  A[API request accepted] --> B[Persist request + outbox + idempotency key]
  B --> C[Consent and suppression evaluation]
  C -->|allowed| D[Resolve template version + locale]
  C -->|blocked| X[Emit SUPPRESSED event + audit]
  D --> E[Publish to priority topic]
  E --> F[Worker claims message]
  F --> G[Provider routing policy check]
  G --> H[Send to provider]
  H --> I{ACK received?}
  I -->|yes| J[Update delivery status]
  I -->|timeout/retryable| K[Retry scheduler]
  K --> H
  I -->|permanent fail| L[DLQ + incident signal]
```
