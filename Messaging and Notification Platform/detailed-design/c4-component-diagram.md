...
## Objective
Provide implementation-ready guidance for **C4 Component Diagram** in the Messaging and Notification Platform.
...
```mermaid
flowchart TB
  API[Notification API] --> Cmd[Command Service]
  Cmd --> Policy[Consent/Policy Engine]
  Cmd --> Outbox[Outbox Publisher]
  Outbox --> Bus[(Message Bus)]
  Bus --> Dispatcher[Dispatch Worker]
  Dispatcher --> Router[Provider Router]
  Router --> Email[Email Adapter]
  Router --> SMS[SMS Adapter]
  Router --> Push[Push Adapter]
  Router --> Webhook[Webhook Adapter]
  Dispatcher --> Status[Delivery Status Service]
  Status --> Audit[Audit Ledger]
  Status --> Metrics[Telemetry Exporter]
```
