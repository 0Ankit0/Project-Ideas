...
## Objective
Provide implementation-ready guidance for **State Machine Diagrams** in the Messaging and Notification Platform.
...
stateDiagram-v2
  ...
  DISPATCHING --> RETRY_SCHEDULED: transient failure
  RETRY_SCHEDULED --> DISPATCHING
  DISPATCHING --> FAILED: permanent failure
  FAILED --> DLQ_RECORDED
  DELIVERED --> [*]
  DLQ_RECORDED --> [*]
```
