...
erDiagram
  ...
  PROVIDER ||--o{ SLOT : owns
  SLOT ||--o| APPOINTMENT : allocated_to
  APPOINTMENT ||--o| PAYMENT_INTENT : funded_by
  APPOINTMENT ||--o{ APPOINTMENT_EVENT : emits
  APPOINTMENT ||--o{ NOTIFICATION_LOG : notifies
...
- Partition `appointment_event` by month and tenant.
- Unique index on `(provider_id, start_at, end_at, state_active)` for conflict prevention.
- Partial index for active appointments (`state in CONFIRMED,CHECKED_IN,IN_CONSULTATION`).
- Soft delete is disallowed for appointment events; append-only audit policy applies.

## Data Retention
- Appointment records: 7 years (configurable by jurisdiction).
- Notification payloads: metadata retained, PHI fields redacted after 30 days.
- Audit logs: immutable storage with WORM retention policy.
