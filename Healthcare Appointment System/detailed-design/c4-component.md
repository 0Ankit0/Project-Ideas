...
```mermaid
flowchart TB
  subgraph SchedulingService[Scheduling Service]
...
## Component Contracts
- **Controller:** validates request schema, auth context, idempotency key.
- **Application Layer:** command orchestration and transaction boundaries.
- **Policy Engine:** cancellation window, no-show fee, eligibility policies.
- **State Machine:** deterministic transition guards and reason-code handling.
- **Outbox Publisher:** exactly-once effect for emitted events through transactional outbox.

## Required Tests
- Component contract tests for all command handlers.
- Mutation tests for transition guard logic.
- Fault-injection tests for outbox retry and dedupe behavior.
