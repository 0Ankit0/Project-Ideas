...
This section captures implementation-level handling for non-happy-path behavior. Each topic defines triggers, expected behavior, fallback actions, and observability signals.

## Coverage Areas
- Slot availability race conditions and stale reads.
- API and UI behavior for retries, duplicates, and partial failures.
- Booking/payment interlocks.
- Cancellations, refunds, and no-show policy disputes.
- Notification delivery failures and consent changes.
- Security/compliance exceptions.
- Operational incidents and downtime reconciliation.

## Cross-Cutting Edge-Case Principles
1. Deterministic error codes for user and operator tooling.
2. Idempotent retries for both clients and internal workers.
3. Human-override workflows with explicit audit trails.
4. No hidden state changes on rejected transitions.
