...
1. Slot reserved but payment authorization fails.
2. Payment authorized but appointment commit fails.
3. Gateway timeout with unknown payment outcome.

## Resolution Rules
- Use saga orchestration: reserve slot -> auth payment -> confirm appointment -> capture/void.
- If payment fails, release slot immediately and return deterministic failure code.
- If appointment fails after auth, issue void/refund automatically and create reconciliation task.

## Reconciliation Jobs
- Sweep orphaned authorizations every 15 minutes.
- Match appointment and ledger records by correlation ID.
- Escalate unresolved mismatches older than 1 hour.

## Compliance Notes
- Store tokenized payment references only.
- Enforce PCI-scoped boundaries and restricted network paths.
