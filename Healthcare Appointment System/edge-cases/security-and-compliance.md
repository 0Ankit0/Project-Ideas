...
## High-Risk Scenarios
- Suspicious account access attempts to patient records.
- Excessive PHI export requests from privileged users.
- Misconfigured retention policy or failed purge jobs.

## Controls and Responses
- Adaptive authentication and risk-based step-up MFA.
- Export throttling and approval workflow for large data pulls.
- Automated retention policy monitors with exception alerts.
- Immediate token/key revocation workflow for compromised credentials.

## Audit and Evidence
- Immutable audit events for data access and policy overrides.
- Signed change records for security configuration updates.
- Quarterly access recertification and least-privilege review.

## Regulatory Alignment
- HIPAA Security Rule safeguards (administrative/technical/physical mapping).
- State-specific patient data rights workflow (access/correction/deletion where applicable).
