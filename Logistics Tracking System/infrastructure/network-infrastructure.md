...
```mermaid
flowchart LR
  Users[Internet Users] --> Edge[WAF + API Gateway]
  Edge --> PubSubnet[Public Subnet: Ingress ALB]
  PubSubnet --> PrivApp[Private Subnet: App Nodes]
  PrivApp --> PrivData[Private Subnet: Datastores]
  PrivApp --> PartnerVPN[Partner VPN/PrivateLink]
  PrivData --> Backup[Cross-Region Backup Network]
  SecOps[Security Tooling] --> PrivApp
  SecOps --> PrivData
```
