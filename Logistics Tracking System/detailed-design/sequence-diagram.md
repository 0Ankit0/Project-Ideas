...
sequenceDiagram
  ...
  Driver->>App: Submit pickup scan
  App->>API: POST /tracking-events (Idempotency-Key)
  API->>SM: validate transition
  SM->>DB: tx(write event + update state + outbox)
  DB-->>SM: commit
  Outbox->>Bus: publish shipment.picked_up.v1
  Bus->>Notif: consume event
  Notif-->>Driver: ACK + customer notification
```
